...
module Comp : COMP = struct
  ...
  let match_pair (c, p) m =
	...
	-> let res = match_value (v, vp) m in
      ...
      | _ -> fprintf stderr "Whoa! No matches: %s with %s\n" (show c)
...
and construct_env_entry hdr env =
  if ENV.mem hdr.mhdr_name env then
    ...
    ENV.add hdr.mhdr_name (fun env cs ->
      fprintf stderr "Evaluating handler %s...\n" hdr.mhdr_name;
...
and eval_var env x = fprintf stderr "Searching for %s...\n" x; return (VInt 2)

and eval_hdr env f = fprintf stderr "Looking for %s...\n" f; ENV.find f env
